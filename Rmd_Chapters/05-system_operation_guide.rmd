...
1. Examine all 20 tanks to make sure each tank has each necessary component:
  ...
  * For a complete list of all items associated with each tank, view the [System Details chapter](03-system_details.rmd).
...
1. Once all tanks are equipped with the above items, make sure:
    ...
    1. The black needle valve for the D flow port (the outflow port for the short outflow pipe, located underneath each tank) is fully closed (turned all the way clockwise, just finger-tight). This ensures the tanks will fill completely initially. If you plan to use tidal cycling, refer to the [Secondary Sump Overflow section](#Overflow_into_Secondary_Sump) for a different N, S, and D flow port configuration.
1. To fill the system with seawater, follow the [water collection guide](06-water_collection.rmd).
    ...
    1. Once the water level of the sump is about 2/3 full, slowly open flow to the mesocosm tanks (again, refer to the [water collection guide](06-water_collection.rmd) for exact positioning. Do not fully open or fully close any valves when redirecting flow to the mesocosm tanks.)
    1. If you are planning to use the system for tidal cycling, please refer to the [Secondary Sump Overflow section](#Overflow_into_Secondary_Sump) for more specific information on this setup.
1. Frequently walk between the mesocosm tanks and the sump to make sure there are no spills or leaks, and that every tank is filling, while also making sure the water level in the sump is not falling too low (should not reach the carbon filters) or too high (should not rise much above the matala filters). 
    1. Before or while the mesocosm tanks are filling, you can also log into ApexFusion and turn ON flow from the S port (via SOL-TNK-# on the home dashboard for each Apex). [Directions for navigating ApexFusion are here.](12-apex_fusion_guide.rmd)
